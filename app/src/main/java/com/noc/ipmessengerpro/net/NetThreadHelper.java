package com.noc.ipmessengerpro.net;
...
import com.noc.ipmessengerpro.activity.MyFeiGeActivity;
import com.noc.ipmessengerpro.activity.MyFeiGeBaseActivity;
import com.noc.ipmessengerpro.data.ChatMessage;
import com.noc.ipmessengerpro.data.User;
import com.noc.ipmessengerpro.interfaces.ReceiveMsgListener;
import com.noc.ipmessengerpro.utils.IpMessageConst;
import com.noc.ipmessengerpro.utils.IpMessageProtocol;
...
public class NetThreadHelper implements Runnable{
	...
	private byte[] resBuffer = new byte[BUFFER_LENGTH];	//接收数据的缓存
	...
	private NetThreadHelper(){
		users = new HashMap<>();
		receiveMsgQueue = new ConcurrentLinkedQueue<>();
	...
	public void run() {
		while(onWork){
			...
			if(udpResPacket != null){	//每次接收完UDP数据后，重置长度。否则可能会导致下次收到数据包被截断。
				udpResPacket.setLength(BUFFER_LENGTH);
			}
	...
	public boolean connectSocket(){	//监听端口，接收UDP数据
		...
		try {
			...
			if(udpResPacket == null)
				udpResPacket = new DatagramPacket(resBuffer, BUFFER_LENGTH);
